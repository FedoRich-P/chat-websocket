            ...
            <ul className="space-y-3 flex-1 overflow-y-auto">
                {users.length === 0 ? (
                    ...
                    users.map((user) => (
                        <li
                            key={user.id}
                            className={`p-2 rounded-md hover:bg-gray-200 cursor-pointer text-gray-700 font-medium ${
                                user.id === mySocketId ? "opacity-50 cursor-default" : ""
                            }`}
                            onClick={() => {
                                if (user.id !== mySocketId && onSelectUser) {
                                    onSelectUser(user);
                                }
                            }}
                        >
                            {user.name} {user.id === mySocketId && "(Вы)"}
                        </li>
                    ))
                )}
            </ul>
